...
public class InputHandler implements KeyListener{
	...
	private boolean beginThrusterSound=false;
	private boolean thrusterSound=false;
	...
	public void handleInput(GameScreen gameScreen){
		...
		if(!status.isGameOver() && !status.isNewShip() && !status.isGameStarting() && status.isGameStarted()){
			...
			if(shiftIsPressed){
				...
				if(!beginThrusterSound)
				{
				soundMan.playBeginThrusterSound();
				beginThusterTime=System.currentTimeMillis();
				beginThrusterSound=true;
				}
				long currentTime = System.currentTimeMillis();
				if(((currentTime - beginThusterTime) > 2950) && !thrusterSound ){
	...
	public void keyReleased(KeyEvent e) {
		...
		case KeyEvent.VK_SHIFT:
			...
			soundMan.stopBeginThrusterSound();
			soundMan.stopThrusterSound();
			beginThrusterSound=false;
			thrusterSound=false;
			break;
		}
		e.consume();
